	...
	Context("with valid upstream spec", func() {
		...
		Context("with secrets", func() {
			...
			It("should have the correct output", func() {
				...
				tlsContext := pluginutils.MustAnyToMessage(out.TransportSocket.GetTypedConfig()).(*envoytls.UpstreamTlsContext)
				Expect(tlsContext.Sni).To(Equal("my-appwhos.azurewebsites.net"))
				Expect(out.GetType()).To(Equal(envoycluster.Cluster_LOGICAL_DNS))
				Expect(out.DnsLookupFamily).To(Equal(envoycluster.Cluster_V4_ONLY))
			})
